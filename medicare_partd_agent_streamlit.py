...
def lookup_partd_plans(zip_code, meds_list):
    """
    If CMS_PLAN_FINDER_KEY is missing or the API call fails, return sample static data.
    Otherwise, call the CMS Plan Finder API to fetch real Part D plans.
    """
    key = st.secrets.get('CMS_PLAN_FINDER_KEY', '')
    if not key:
        st.warning("No CMS API key found; displaying sample plans.")
        # Return sample data for MVP/demo
        return [
    ...
    try:
        ...
        if not plans:
            st.info("No plans returned from CMS; using sample plans.")
            return lookup_partd_plans(zip_code, meds_list)
        return plans
...
st.markdown("---")
st.caption("MVP prototype: ZIP + meds to Part D lookup + LLM summary (or sample data)")
